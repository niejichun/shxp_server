...
exports.ERCOrderControlResource = (req, res) => {
    ...
    } else if (method==='setOrderToComplete'){
        setOrderToCompleteAct(req,res)
    }  else {
...
// 订单完成
async function setOrderToCompleteAct(req,res){
    try {
        let doc = common.docTrim(req.body), user = req.user, returnData = {},replacements=[];
        ...
        if (order.order_state !== 'WORKING') {
            return common.sendError(res, 'order_04');
        }
        ...
        if (napCount === 0) { //全部验收项通过订单既是已完成
            order.order_state = 'FINISHI';
            await order.save();
            ...
            let pointtype = await tb_pointtype.findOne({
                where: {
                    pointtype_id: '3',
                    customerpoint_name: '订单完成',
                    state:1
                }
            });
            ...
            if (pointtype) {
                ...
                if(customer){
                    customer.customer_point = customer.customer_point + order.final_offer*(pointtype.base_point/1000);
                    await customer.save();
            ...
            return common.sendError(res, 'order_05');
        }
        common.sendData(res, returnData);
    } catch (error) {
        common.sendFault(res, error);
