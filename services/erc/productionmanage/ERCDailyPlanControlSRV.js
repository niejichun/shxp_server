...
// 生产日计划接口
exports.ERCDailyPlanControlResource = (req, res) => {
    let method = req.query.method;
    if (method==='getDailyPaln'){
        getDailyPaln(req,res)
    }else if (method==='init'){
        initAct(req,res)
...
// 获得日计划列表
async function getDailyPaln(req,res){
    let queryStr,replacements=[],returnData={};
    let doc=common.docTrim(req.body),user=req.user;
    let beginDate =moment().add('days',1).format('YYYY-MM-DD');
