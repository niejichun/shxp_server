...
const client = JPush.buildClient('b0ad4d04704b9fd0e5378e2c', 'e64522a13420604230952060');
...
async function querySimple(db, req, queryStr, replacements) {
    let doc = req.body
...
function exceltojson(uploadurl) {
    return new Promise(function(resolve, reject) {
        try {
            let fileName = path.basename(uploadurl);
            let tempfile = path.join(__dirname, '../' + config.uploadOptions.uploadDir + '/' + fileName);

            let workbook = XLSX.readFile(tempfile);
...
function pushNotification(alertTitle, alertContent, extras, userId) {
...
function searchKujialeCityid(province, city) {
    let citys = require('../kujiale/cities.json')
    for (let p of citys) {
        if (province.search(p.province) >= 0) {
            for (let c of p.cities) {
                if (city.search(c.name) >= 0) {
                    return c.cityid
                }
            }
        }
...
module.exports = {
    ...
    searchKujialeCityid: searchKujialeCityid
};
